...
const { AficineScraper } = require('./src/services/scraping/scrapers/AficineScraper');
const { CineCiutatScraper } = require('./src/services/scraping/scrapers/CineCiutatScraper');
const { MajorcaDailyBulletinScraper } = require('./src/services/scraping/scrapers/MajorcaDailyBulletinScraper');
const { CinesMoixNegreScraper } = require('./src/services/scraping/scrapers/CinesMoixNegreScraper');
...
async function testScraper(scraper, name) {
  ...
  try {
    ...
    if (result.data.length > 0) {
      ...
      result.data.slice(0, 3).forEach((showtime, idx) => {
