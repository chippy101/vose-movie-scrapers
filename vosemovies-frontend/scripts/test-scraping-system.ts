...
import { EnhancedVOSEDetector } from '../src/services/scraping/detectors/EnhancedVOSEDetector';
import { DataValidationSystem } from '../src/services/validation/DataValidationSystem';
import { ScrapedShowtime } from '../src/types/Cinema';
...
const test1 = detector.detectVOSE(
  'Oppenheimer - VOSE - 20:30 at CineCiutat',
  undefined,
  'Oppenheimer',
  undefined,
  true // Enable debug
...
const test2 = detector.detectVOSE(
  'Oppenheimer - Doblada al Español - 18:00',
  undefined,
  'Oppenheimer'
);
...
const test3 = detector.detectVOSE(
  'Poor Things - VOSE 20:30, Doblada 18:00 at Cinesa',
  undefined,
  'Poor Things'
);
...
const test4 = detector.detectVOSE(
  'The Zone of Interest showing in Palma, Mallorca at 21:00',
  undefined,
...
const validShowtime: ScrapedShowtime = {
...
const invalidShowtime: ScrapedShowtime = {
...
const duplicateShowtime: ScrapedShowtime = {
  ...validShowtime,
  startTime: new Date('2025-10-14T20:31:00') // Within 1 minute
};

async function runValidationTests() {
  // Test valid showtime
  const result1 = await validator.validateShowtimes([validShowtime]);
  ...
  const result2 = await validator.validateShowtimes([invalidShowtime]);
  ...
  const result3 = await validator.validateShowtimes([validShowtime, duplicateShowtime]);
  ...
  const result4 = await validator.validateShowtimes([
    validShowtime,
    invalidShowtime,
    duplicateShowtime
  ]);
